...
postfile `rep_estimates' str20 confounding str20 getallpossible int(rep_id) int(ratio) int(method) float(nomatch theta se lci uci pval) using rep_mysimestimates_`type_ith'_sim${nsim_start}_${nsim}_${conf_type}.dta, replace 
...
postfile `rep_states' str10 confounding int(rep_id) str20 implementation int(ratio) str2000 state1 str2000 state2 str1000 state3 using rep_mysimstates_`type_ith'_sim${nsim_start}_${nsim}_${conf_type}.dta, replace ///figure this out later

tempname cohort_deets
postfile `cohort_deets' str20 confounding str20 getallpossible int(rep_id) int(ratio) int(n_set) int(median_set) int(set_25) int(set_75) float(full_sets) float(time_mtch) using rep_cohortdeets_`type_ith'_sim${nsim_start}_${nsim}_${conf_type}.dta, replace
