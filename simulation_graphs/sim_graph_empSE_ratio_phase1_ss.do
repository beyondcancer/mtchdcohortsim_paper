...
|| rcap MCSE_lo MCSE_hi ratio_offset_6 if method == 7 & getallpossible == "replacement", mcol("0 191 111") lcol("0 191 111") lpattern(dash) yline(0, lcol(black))  ///
	...
	xlab(1 "1:1" 3 "3:1" 5 "5:1" 10 "10:1", labsize(medium)) ylab(, format(%9.0g) nolabels) name(empSE_ca_norep, replace) ///
	legend(off)
...
|| rcap MCSE_lo MCSE_hi ratio_offset_6 if method == 13 & getallpossible == "replacement", mcol("0 191 111") lcol("0 191 111") lpattern(dash) yline(0, lcol(black))  ///
	xtitle("") title("Cox models", size(small)) ///
	xlab(1 "1:1" 3 "3:1" 5 "5:1" 10 "10:1", labsize(medium)) ylab(, format(%9.0g) nolabels) name(empSE_ct_norep, replace) ///
	legend(off)
...
|| rcap MCSE_lo MCSE_hi ratio_offset_6 if method == 10 & getallpossible == "replacement", mcol("0 191 111") lcol("0 191 111") lpattern(dash) yline(0, lcol(black)) ///
	...
	xlab(1 "1:1" 3 "3:1" 5 "5:1" 10 "10:1", labsize(medium)) ylab(, format(%9.0g) labsize(medium)) 	name(empSE_pa_norep, replace) ///
	legend(off)
...
|| rcap MCSE_lo MCSE_hi ratio_offset_6 if method == 16 & getallpossible == "replacement", mcol("0 191 111") lcol("0 191 111") lpattern(dash) yline(0, lcol(black)) ///
	xtitle("") title("Poisson models", size(small)) ///
	xlab(1 "1:1" 3 "3:1" 5 "5:1" 10 "10:1", labsize(medium)) ylab(, format(%9.0g) labsize(medium)) 	name(empSE_pt_norep, replace) ///
	legend(off)
...
graph combine empSE_pt_norep empSE_ct_norep, imargin(0 0 0 0) xcommon ycommon title("Time in study", size(small)) name(empSE_t_`conf'_ratio, replace) //legendfrom(empSE_p_${getallpossible})

graph combine empSE_pa_norep empSE_ca_norep, imargin(0 0 0 0) xcommon ycommon title("Age", size(small)) name(empSE_a_`conf'_ratio, replace) //legendfrom(empSE_p_${getallpossible})

graph combine empSE_t_`conf'_ratio empSE_a_`conf'_ratio, name(empSE_`conf'_ratio, replace) title("Empirical SE") row(2) cols(2)
...
//graph combine empSE_10_smallco_ratio empSE_7_smallcon_ratio empSE_5_smallcon_ratio, xcommon ycommon imargin(0 0 0 0) c(3) name(empSE_all_smallconf, replace) title("Relative % error of the Model SE", size(small))
 ...
 graph combine empSE_t_10_sconf_ratio empSE_a_10_sconf_ratio empSE_t_1_sconf_ratio empSE_a_1_sconf_ratio, xcommon ycommon imargin(0 0 0 0) c(2) r(2) name(empSE_all_sconf, replace) title("Empirical SE", size(small))
 
graph export "$resultsDir\sim_graph_empSE_ratio_all_smallconf_${nsim}_${type}.emf", replace


graph combine empSE_t_10_nconf_ratio empSE_a_10_nconf_ratio empSE_t_1_nconf_ratio empSE_a_1_nconf_ratio, xcommon ycommon imargin(0 0 0 0) c(2) r(2) name(empSE_all_nconf, replace) title("Empirical SE", size(small))
 
 
graph export "$resultsDir\sim_graph_empSE_ratio_all_noconf_${nsim}_${type}.emf", replace


graph combine empSE_t_10_bconf_ratio empSE_a_10_bconf_ratio empSE_t_1_bconf_ratio empSE_a_1_bconf_ratio, xcommon ycommon imargin(0 0 0 0) c(2) r(2) name(empSE_all_bconf, replace) title("Empirical SE", size(small))
 
 
graph export "$resultsDir\sim_graph_empSE_ratio_all_bconf_${nsim}_${type}.emf", replace