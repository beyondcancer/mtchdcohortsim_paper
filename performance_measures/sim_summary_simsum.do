...
simsum theta, se(se) methodvar(method) id(rep_id) true($true) mcse format(%6.3f %6.1f %6.0f) bsims bias empse modelse relerror cover listsep by(confounding ratio getallpossible) saving($resultsDir\sim_summary_simsummtch_${nsim}_${type}, replace)
...
simsum theta, se(se) methodvar(method) id(rep_id) true($true) mcse format(%6.3f %6.1f %6.0f) listsep bsims bias empse modelse relerror cover by(confounding) saving($resultsDir\sim_summary_simsumbc_${nsim}_${type}, replace)
...
save "$resultsDir//simsum_mtch_numbers_${nsim}_${type}", replace
...
save "$resultsDir//simsum_bc_numbers_${nsim}_${type}", replace
